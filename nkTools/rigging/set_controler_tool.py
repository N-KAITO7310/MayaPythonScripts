...
要件
・既存のコントローラ作成機能の拡張版
・ウィンドウでのオプション選択ができるようにする
・複数ジョイントを選択し、ジョイントが階層関係にある場合はコントローラも階層化する、オンオフ可能
・コンストレインの種類選択
・サイズ
・形状選択

from nkTools.rigging import set_controller_tool;
reload(set_controller_tool);
reload(set_controller_tool.qt);
set_controller_tool.option();

制作日:8/11
制作時間:5h
"""

def setControllerTool():
    ...
    for index, selected in enumerate(selectedObjects):
        ...
        if(settings.shape == 0):
            ...
            cmds.circle(c=(0, 0, 0), nr=(1, 0, 0), sw=360, r=radius, d=3, ut=0, tol=0.01, s=8, ch=0, n=controllerName);
        elif(settings.shape == 1):
            # 四角
            point =settings.diameter;
            cmds.curve(d=1, p=[(point, 0, point*-1), (point*-1, 0, point*-1), (point*-1, 0, point), (point, 0, point), (point, 0, point*-1)], k=[0, 1, 2, 3, 4], n=controllerName);
        elif(settings.shape == 2):
            # 三角
            pointA = 1.03923 * settings.diameter;
            pointB = 0.6 * settings.diameter;
            pointC = 1.2 * settings.diameter;
            cmds.curve(d=1, p=[(pointA*-1, 0, pointB), (pointA, 0, pointB), (0, 0, pointC*-1), (pointA*-1, 0, pointB)], k=[0, 1, 2, 3], n=controllerName);
        ...
        if(settings.constrainOption == 0):
            cmds.parentConstraint(controllerName, str(selected), mo=False);
        elif(settings.constrainOption == 1):
            cmds.pointConstraint(controllerName, str(selected), mo=False);
        elif(settings.constrainOption == 2):
            cmds.orientConstraint(controllerName, str(selected), mo=False);
        else:
            cmds.orientConstraint(controllerName, str(selected), mo=False);
        
        # 作成データの相関を保持
        createdCurveAndGroup[jointName] = [controllerName, str(offsetGrp)];
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Set Contoroller Tool");
        self.resize(500, 300);
