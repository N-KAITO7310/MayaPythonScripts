...
ScriptEditorUse:
from nkTools.rigging import setFKIKsplineControllerTool;
reload(setFKIKsplineControllerTool);
reload(setFKIKsplineControllerTool.qt);
setFKIKsplineControllerTool.option();
...
def setFKIKsplineController():
    ...
    cmds.rename(splineCurve, splineIkName)
    # off inheritsTransform
    cmds.setAttr("{}.inheritsTransform".format(splineCurve), 0);
    cmds.select(clear=True);

    # place ikCtrlJnts root middle end position
    ...
    for i, ikSplineJoint in enumerate(curveControleJointList):
        # main prefix
        ...
        # create offset_grp and parent
        OFFSET_SUFFIX = "_offset_grp";
        AUTO_SUFFIX = "_auto_grp";
        offsetGrp = str(cmds.group(n=prefix + OFFSET_SUFFIX, em=True));
        autoGrp = str(cmds.group(n=prefix + AUTO_SUFFIX, p=offsetGrp, em=True));
        pm.matchTransform(offsetGrp, controllerName, pos=True, rot=True);
        pm.matchTransform(autoGrp, controllerName, pos=True, rot=True);
        cmds.parent(controllerName, autoGrp);

        # snap jnt by constraint
        cmds.delete(str(cmds.parentConstraint(str(ikSplineJoint), str(offsetGrp), mo=False, weight=1))[0]);

        # parent const
        cmds.parentConstraint(controllerName, str(ikSplineJoint), mo=False);

        # preserve info for hierarchy
        createdIkCurveAndGroup[i] = [controllerName, str(offsetGrp)];

    # IkSpline Twist Setting
    # activate advanced twist control
    ...
    # set twist worldUp
    ikRootCtrl = createdIkCurveAndGroup[0][0];
    ikEndCtrl = createdIkCurveAndGroup[2][0];
    cmds.connectAttr(ikRootCtrl + ".worldMatrix[0]", splineIkName + ".dWorldUpMatrix", f=True);
    cmds.connectAttr(ikEndCtrl + ".worldMatrix[0]", splineIkName +  ".dWorldUpMatrixEnd", f=True);
    
    # setting stretch
    ikCurveInfo = cmds.createNode("curveInfo", n="{}_ikCurve_info".format(prefix));
    arcLength = cmds.getAttr(str(ikCurveInfo) + ".arcLength");
    curveLengthMd = cmds.shadingNode('multiplyDivide', asUtility=True, name="curveLength_MD");

    cmds.setAttr(curveLengthMd + ".operation", 2);
    cmds.connectAttr(str(ikCurveInfo) + ".arcLength", curveLengthMd + ".input1X", f=True);
    ...
    # set fk jnt
    # dict = jointName:[curveName, offsetGroupName]
