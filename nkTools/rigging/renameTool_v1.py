...
"""
親group名とオブジェクトタイプに応じて、配下のオブジェクトを一括リネームする

選択オブジェクトの取得とチェック
選択オブジェクトがgroupであればその配下のオブジェクトを取得
配下のオブジェクトがトランスフォームノードであり、curve・jnt・geoのどれかに該当するものであれば、名称変更の対象にする
spine1_jntのような形式でgroup名_suffixでリネームする
配下のオブジェクトがgroupであり、さらに下の階層を持つ場合にさらにそれ以下も同様の処理を行う（ただし、すでに上記いずれかの接尾辞を持つ場合は適用しない

from nkTools.rigging import renameTool_v1;
import importlib
importlib.reload(renameTool_v1);
renameTool_v1.showUi();
"""
def maya_useNewAPI():
