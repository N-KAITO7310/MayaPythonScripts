...
CreateFixedIkSplineSystemTool

created: 2022/10/27

開始ジョイント～終了ジョイントを選択し、実行。ギミック参考をもとにしたIKsplineリグをセットアップする。
...
property:
transformCtrl: ルートコントロールオブジェクト、スケールの接続元として使用

ScriptEditorUse:
from nkTools.rigging import createFixedIkSplineSystemTool;
reload(createFixedIkSplineSystemTool);
createFixedIkSplineSystemTool.showUi();
