...
"""
ImportExportMIrrorPoseTool

description: ポーズのImport/Export/Mirrorを行うツール

version: 1.2
created Date: 2022/02/01
last update: 2023/03/25
updateContents 2023/03/25:
・ミラー対象からfrontArmorikのコントローラを削除。
...
update予定
・表示ウィジェットによる画像表示(Qlabel)
・import export でタブを分ける(QtabWidget)
・ファイル表示をリストし、選択することでインタラクティブに画像を表示する
・プログレスバーの表示
・特殊なリグ構成でない限りある程度対応できるよう汎用化

import nkTools.animation.importExportMIrrorPoseTool as imp;
reload(imp);
reload(imp.qt);
imp.option();
...
def mirrorPose():
    ...
    2. 回転軸が同じor中心にあるコントローラについてはCoyoteブログの手法を利用して反転Rotを取得して、Proxyに接続後、該当コントローラに流す
