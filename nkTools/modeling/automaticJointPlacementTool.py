...
"""
AutomaticJointPlacementTool

description:
同一のトポロジーを持つモデルに対し、保存したジョイント情報から再配置を行う。

created: 23/06/13

下記を参考し、一部可読性のため修正：
https://www.cgcircuit.com/tutorial/auto-place-skeletons-in-any-character


dirpath = r'C:\Users\kn_un\Documents\maya\scripts\nkTools\tool_resource'
import sys, imp 
sys.path.insert(0, dirpath) 
import nkTools.modeling.automaticJointPlacement as autoJ
imp.reload(autoJ)
autoJ.openUI(dirpath)
...
class Skeletor(object):
    ...
    def record_all(self, sel_Array=None):
        ...
        #...load_data
        self.load_data(mesh, node_Array);
        ...
        #...debug
        print("ERROR: Recorded nodes on mesh: '%s'"%mesh);
        print(node_Array[0]);
    ...
    def record_one(self, sel_Array=None):
        ...
        if sample_Array == []:
            ...
            return False;
        print(node_array, mesh, sample_Array)
        #...load_data
    ...
    def mirror_records(self, sel_Array=None):
        ...
        #...mirror_all
        self.mirror_all(mesh, self.data.keys());
    ...
    def place_one(self, mesh, node):
        ...
        cmds.select(cl=True);
        cmds.xform(node, t=pos_nodeNew, ws=True);
...
def rotate_planar(node_Array = None):
        ...
        vecA_pos = cmds.xform(nodeA, scalePivot=True, query=True, worldSpace=True);
        vecB_pos = cmds.xform(nodeB, scalePivot=True, query=True, worldSpace=True);
        vecC_pos = cmds.xform(nodeC, scalePivot=True, query=True, worldSpace=True);
        
        #...get vectors and matricies
        vecAx = get_vector(vecA_pos, vecB_pos);
        vecBx = get_vector(vecB_pos, vecC_pos);

        vecAy = crossProduct(vecAx, vecBx);
        vecAz = crossProduct(vecAy, vecAx);
        
        vecBy = crossProduct(vecAx, vecBx);
        vecBz = crossProduct(vecBy, vecBx);
        
        vecAx = normalize_Array(vecAx);
        vecAy = normalize_Array(vecAy);
        vecAz = normalize_Array(vecAz);
        
        nodeA_matrix = create_matrix(vecAx, vecAy, vecAz, position=vecA_pos);
        nodeB_matrix = create_matrix(vecBx, vecBy, vecBz, position=vecB_pos);
        
        #...orient nodes
        cmds.xform(nodeA, m=nodeA_matrix, worldSpace=True);
        cmds.xform(nodeB, m=nodeB_matrix, worldSpace=True);
        
        cmds.makeIdentity(nodeA, apply=True, t=True, r=True, s=True, n=0);
        cmds.makeIdentity(nodeB, apply=True, t=True, r=True, s=True, n=0);
